...
import SendPrefilledMessageButton from './SendPrefilledMessageButton';
...
const ChatForm = memo(({ index = 0 }: { index?: number }) => {
  ...
  const isLastestMessageCreatedByUser = useMemo(
    () => (latestMessage ? !!latestMessage.isCreatedByUser : true),
    [latestMessage],
  );
  ...
  const { submitMessage, submitPrompt, submitPrefilledMessage, submitPrefillAsk, submitAddTitle } =
    useSubmitMessage();
  ...
  const handlePrefillAskCallback = useCallback(() => {
    console.log('PORRA - 0 - handlePrefillAskCallback');
    submitPrefillAsk();
  }, [submitPrefillAsk]);
            ...
            >
              <div className={`${isRTL ? 'mr-2 flex items-center' : 'ml-2 flex items-center'}`}>
                ...
                {endpoint && prefillMessages && (
                  <>
                    <SendPrefilledMessageButton
                      handlePrefillAsk={handlePrefillAskCallback}
                      disabled={
                        filesLoading ||
                        isSubmitting ||
                        disableInputs ||
                        isLastestMessageCreatedByUser
                      }
                    />
