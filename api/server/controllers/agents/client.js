...
class AgentClient extends BaseClient {
  ...
  async buildMessages(
    ...
    console.log(
      `[AgentClient/buildMessages/client.js] orderedMessages: ${inspect(orderedMessages, { depth: 5, colors: false })}`,
    );
  ...
  async chatCompletion({ payload, abortController = null }) {
    ...
    try {
      ...
      console.log(
        `api/server/controllers/agents/client.js - chatCompletion - payload: ${JSON.stringify(payload)}`,
      );
      ...
      for (const initialMsg of initialMessages) {
        console.log(
          `api/server/controllers/agents/client.js - chatCompletion - initialMessages: ${JSON.stringify(initialMsg)}`,
        );
      }
      ...
      const runAgent = async (agent, _messages, i = 0, contentData = [], _currentIndexCountMap) => {
        ...
        console.log(
          `[controllers/agents/client.js runAgent] messages: ${inspect(messages, { depth: 4, colors: false })}`,
        );
      ...
      console.log(
        `[controllers/agents/client.js initializeClient] this.options.agent: ${JSON.stringify(this.options.agent)}`,
      );
      console.log(
        `[controllers/agents/client.js initializeClient] initialMessages: ${JSON.stringify(initialMessages)}`,
      );
