...
class BaseClient {
  ...
  async setAddMessageOptions(opts) {
    ...
    console.log(
      `PORRA - 0 - setAddMessageOptions - saveOptions: ${inspect(saveOptions, { depth: 3, colors: true })}`,
    );
    ...
    console.log(
      `PORRA - 8 - setAddMessageOptions - this.currentMessages: ${inspect(this.currentMessages, { depth: 3, colors: true })}`,
    );
  ...
  async addPrefilledMessage(message, opts) {
    ...
    console.log(
      `PORRA - 0 - addPrefilledMessage - userMessage: ${inspect(userMessage, { depth: 3, colors: true })}`,
    );
    ...
    console.log(
      `PORRA - 1 - addPrefilledMessage - userMessage: ${inspect(userMessage, { depth: 3, colors: true })}`,
    );
    ...
    try {
      ...
      console.log('PORRA - 2 - addPrefilledMessage - this.options.attachments DONE');
    } catch (error) {
    ...
    console.log(
      `PORRA - 3 - addPrefilledMessage - userMessage: ${inspect(userMessage, { depth: 3, colors: true })}`,
    );
    // if (this.options.attachments) {
    //   userMessage.files = this.options.attachments;
    //   // conversation.model = endpointOption.modelOptions.model;
    //   delete userMessage.image_urls;
    //   console.log('PORRA - 2.1 - addPrefilledMessage - this.options.attachments DONE');
    // }

    // if (!isEdited && !this.skipSaveUserMessage) {
    // const userMessagePromise = this.saveMessageToDatabase(userMessage, saveOptions, user);
    ...
    console.log(
      `PORRA - 4 - addPrefilledMessage - this.currentMessages: ${inspect(this.currentMessages, { depth: 3, colors: true })}`,
    );
    ...
    if (
      ...
      await this.recordTokenUsage({ promptTokens, completionTokens: 0, usage: null });
    }
    ...
    console.log(
      `PORRA - 5 - addPrefilledMessage - saveOptions: ${inspect(saveOptions, { depth: 3, colors: true })}`,
    );
    ...
    console.log(
      `PORRA - 6 - addPrefilledMessage - userMessage: ${inspect(userMessage, { depth: 3, colors: true })}`,
    );
